...
function finalizeRow(
  ...
  if (statusSelecionado === "todos") {
    ...
    row.horasDisponivel = filtroInclui(statusSelecionado, "todos", "Disponível")
      ? row.horasTotaisOperacao
      : 0;
    ...
    if (
      !filtroInclui(statusSelecionado, "todos", "Ocorrência operacional") &&
      !filtroInclui(statusSelecionado, "todos", "Com falha ou parcial")
...
function aggregateData(filtros: {
  ...
  const mensal = [...mensalMap.values()].filter((row) => {
    ...
    if (filtros.status !== "todos") {
      ...
      if (
        !filtroInclui(filtros.status, "todos", "Ocorrência operacional") &&
        !filtroInclui(filtros.status, "todos", "Com falha ou parcial")
      ) row.horasFalhaParcial = 0;
...
export default function DashboardOcorrencias2025({
  ...
  const incluirDadosIndisponiveisNosGraficos =
    status !== "todos" && filtroInclui(status, "todos", "Indefinido");
    ...
    <main className={`page dashboard-page ${isAnoPendente ? "is-route-changing" : ""}`}>
      ...
      <section className="filters-panel">
        ...
        <div className="filters-grid">
          <GlobalMultiSelect
            ...
            selected={valoresDoFiltro(linha, "todas")}
            onChange={(valores) => setLinha(valorDoFiltro(valores, "todas"))}
            options={LINHAS.map((item) => ({
              value: item,
              label: rotuloNomeLinha(item),
              visual: <LineBadge nome={item} className="global-multi-select-line-badge" />,
            }))}
          />
          ...
          <GlobalMultiSelect
            ...
            selected={valoresDoFiltro(operador, "todos")}
            onChange={(valores) => setOperador(valorDoFiltro(valores, "todos"))}
            options={OPERADORES.map((item) => ({
              value: item,
              label: displayOperadorName(item),
            }))}
          />
          ...
          <GlobalMultiSelect
            ...
            selected={valoresDoFiltro(status, "todos")}
            onChange={(valores) => setStatus(valorDoFiltro(valores, "todos"))}
            options={ESTADOS_FILTRO_GLOBAL.map((item) => ({
              value: item,
              label: item,
              visual: <span className="global-multi-select-status-dot" style={{ "--status-color": CORES[item] } as CSSProperties} aria-hidden="true" />,
            }))}
          />
      ...
      <section className="hero-card operational-legend-line" aria-label="Legenda operacional">
        ...
        <div className="dashboard-legend-sections">
          <section className="dashboard-legend-section" aria-labelledby="operational-status-legend-title-2025">
            <h3 id="operational-status-legend-title-2025">Estados operacionais</h3>
          ...
          <section className="dashboard-legend-section" aria-labelledby="line-legend-title-2025">
            <h3 id="line-legend-title-2025">Linhas</h3>
